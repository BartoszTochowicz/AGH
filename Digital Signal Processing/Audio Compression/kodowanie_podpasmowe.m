...
energies = sum(abs(sb).^2, 2);
total_energy = sum(energies);
bit_allocation = round((energies / total_energy) * sum(q));  % q = łączna liczba bitów
bit_allocation = max(bit_allocation, 1);  % Zapewnij min. 1 bit na pasmo
bps_per_band = bit_allocation;
