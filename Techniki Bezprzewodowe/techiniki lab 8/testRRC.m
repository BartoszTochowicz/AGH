...
numStr1 = zeros(numSyms,1);
numStr2= zeros(numSyms,1);
for i = 1:numSyms
    numStr2(i,1) = randi(M)-1;
    numStr1(i,1) = randi(M)-1;
end
%TODO 2: przekształć wejściowe strumienie danych na ciąg symboli QPSK, użyj funkcji qammod

symStr1 = qammod(numStr1,M); %wygeneruj pierwszy sygnał M-QAM
symStr2 = qammod(numStr2,M); %wygeneruj drugi sygnał M-QAM
...
rrcFlt=rcosdesign(0.35,10,sps);%zmiana w TODO6% #1
...
% symStr1= repelem(symStr1,sps); %zmiana w TODO6% #2
symStr1= upfirdn(symStr1,rrcFlt,1,1);
%symStr2 = repelem(symStr2,sps); %zmiana w TODO6% #2
symStr2 = upfirdn(symStr2,rrcFlt,1,1);
...
detSig = fshift(compositeSig,f1,1/Ts);
...
detSyms=upfirdn(detSig,rrcFlt,1,sps);%redukujemy szybkosc probkowania sps krotnie %zmiana w TODO6% #3. Wykonując zadanie TODO6#3 uzyj funkcji upfirdn
